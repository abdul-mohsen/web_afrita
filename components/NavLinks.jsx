...
const NavLinks = () => {
  ...
  const navHandleClick = (index) => {
    const currentNavLink = document.querySelector(`.nav-link.bg-primary`);
    if (currentNavLink) {
      ...
      {navLinks.map((item, index) => (
        <Link
          ...
          className={`${
            ...
            (currentPath === item.href || `/${isParentPath}` === item.href)
        ...
        >
          <div className="px-3 lg:px-6 py-3 lg:inline-flex items-center lg:w-full text-lg font-semibold transition-colors duration-150 hover:text-white">
            ...
            <span className="hidden lg:block count mr-auto">
              {" "}
              {item.count}{" "}
            </span>
          </div>
