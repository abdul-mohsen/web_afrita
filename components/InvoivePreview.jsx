...
const InvoivePreview = ({ togglue, data }) => {
  ...
  const result = updatedList.map(item => {
    ...
    vatAmount = round(priceBeforeVat * .15, 3)
    total = round(vatAmount + priceBeforeVat, 2)
    return { unitPrice: unitPrice, priceBeforeVat: priceBeforeVat, vatAmount: vatAmount, total: total, productName: productName, quentity: quentity }
  });

  const total = round(result.reduce((a, i) => a + i.total, 0), 2)
  const subtotal = round(result.reduce((a, i) => a + i.priceBeforeVat, 0), 2)
  const vatTotal = round(result.reduce((a, i) => a + i.vatAmount, 0), 2)
    ...
    <div id="invoice-preview" className="fixed flex w-screen h-screen overflow-y-auto overflow-x-hidden top-0 right-0 z-40 justify-center items-start padding" >
      ...
      <div className='relative z-50 bg-white w-full md:w-fit p-8 rounded-lg' >
        <form id='form-preview' action="post" className="max-w-[600px]">
          <h1 className='text-primary text-2xl text-center'>فاتورة ضريبية مبسطة</h1>
            ...
            <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-4 pb-6">
              ...
              <div className=" invoice-row sm:col-span-4 flex flex-col bg-white">
                <div
                  ...
                  <div className="details flex-1 grid grid-cols-6 text-app-gray  py-6">
                    ...
                    <span className="item-place">العسر شامل ضريبة القيمة المضافة</span>
                  </div>
                </div>
                <ul>
              ...
              <div className="mt-10 col-span-4 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-4 border-b border-gray-900/10 pb-12">
                <div className=" flex flex-row flex-wrap col-start-3 col-end-5 gap-x-6 items-center ">
                  <label htmlFor="total" className="block text-lg font-medium leading-6 text-gray-900 flex-1">اجمالي المبلغ الخاضع للضريبة</label>
                ...
                <div className=" flex flex-row flex-wrap col-start-3 col-end-5 gap-x-6 items-center">
                  <label htmlFor="tax" className="block text-lg font-medium leading-6 text-gray-900 flex-1">ضريبة القيمة المضافة(%15)</label>
                  <div className="mt-2 flex-1 dasshed-all p-1 rounded-md">
                    <input
                      id="tax"
                      name="tax"
                ...
                <div className=" flex flex-row flex-wrap col-start-3 col-end-5 gap-x-6 items-center">
                  <label htmlFor="total-amount" className="block text-lg font-medium leading-6 text-gray-900 flex-1">المجموع مع الضريبة(%15)</label>
                  <div className="mt-2 flex-1 dasshed-all p-1 rounded-md">
                    <input
                      id="total-amount"
                      name="total-amount"
