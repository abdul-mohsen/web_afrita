...
import { HiOutlineAdjustmentsHorizontal, HiOutlineArrowRightOnRectangle, HiUser } from 'react-icons/hi2';
  ...
  return (
    <nav className="h-[108px] flex justify-evenly flex-row-reverse gap-6 ps-6 padding-l bg-white w-full">
      <div ref={dropRef} className="user flex justify-between flex-row items-center gap-6">
        ...
        <button onClick={userInfo}   className="user-info relative flex flex-row justify-center items-center gap-2">
          <Image src={AvatarImage} alt="User Image" id='user_image' />
          <div className={`${isUserInfo ? "flex flex-col gap-2" : "max-lg:hidden" } max-lg:absolute top-full max-lg:p-3 max-lg:rounded-md max-lg:bg-app-light-gray max-lg:w-max max-lg:shadow-lg max-lg:z-40  max-lg:mt-2 left-0`}>
            <Link href={'/'} className={`${isUserInfo ? "flex flex-row justify-between items-center gap-8 " : "" } user-name hover:text-primary`} id='user_name'>
      ...
      <div ref={dropReff} className="search-input relative flex-1 flex justify-center items-center bg-white max-md:pe-0 px-0">
        <label htmlFor="search" onClick={openSearchBar}  className={`${isSearchBar ? "flex-1 max-lg:rounded-md" : "max-lg:rounded-full"} rounded-md lg:flex-1 lg:w-full w-[70px] overflow-hidden ml-auto flex flex-row justify-between items-center px-6 py-3 bg-app-light-gray cursor-text`}> 
